...
public final class AntPathRequestMatcher implements RequestMatcher, RequestVariablesExtractor {
    ...
    public AntPathRequestMatcher(String pattern, String httpMethod, boolean caseSensitive){
        ...
        if (pattern.equals(MATCH_ALL) || pattern.equals("**")){
            ...
            //如果模式以{@code / **}结尾，并且没有其他通配符或路径变量，则优化为子路径匹配
            if (pattern.endsWith(MATCH_ALL) && (pattern.indexOf("?") == -1 && pattern.indexOf("{") == -1  && pattern.indexOf("}") == -1) && pattern.indexOf("*") == pattern.length() - 2){
                this.matcher = new SubPathMatcher(pattern.substring(0, pattern.length() - 2), caseSensitive);
            }else {
    ...
    public boolean matches(HttpServletRequest request) {
        if (this.httpMethod != null && StringUtils.hasLength(request.getMethod()) && this.httpMethod != valueOf(request.getMethod())){
